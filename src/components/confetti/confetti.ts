import { Particle } from '../../types/ParticlePhysics';
...
const updateParticle = $(
  (particle: Particle, progress: number, decay: number) => {
    const {
      x,
      y,
      tiltAngle,
    ...
    const wobbleX =
      ...
      (factors[differentiator] * progress * wobble * wobble +
        20 *
          (differentiator % 2 ? Math.sin(wobble / 4) : Math.cos(wobble / 4)));
...
export const confetti = $(
  async (
    ...
    const spanElements = await createElements(
      root,
      elementCount,
      elementSize,
      zIndex,
